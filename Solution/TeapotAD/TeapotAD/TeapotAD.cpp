...
// to check for R to reset camera, to check for A to increase light attenuation,
// to check for S to decrease light attenuation
/////////////////////////////////////////////////////////////////////////////////////////////
static void key_callback(GLFWwindow* window, int key, int cancode, int action, int mods)
{
	if (key == GLFW_KEY_SPACE && action == GLFW_RELEASE)
	{
		if (scene) // If scene has been intialised, do the following
		{
			scene->animate(!(scene->animating())); // Toggle function animate, on release of key 'space'
		}
	}

	if (key == 'R' && action == GLFW_RELEASE)
	{
		camera.reset(); // Call function reset, on release of key 'R'
	}

	if (key == 'A' && action == GLFW_REPEAT)
	{
		scene->attenuationUp(); // Call function attenuationUp, on press of key 'A'
	}
	if (key == 'S' && action == GLFW_REPEAT)
	{
		scene->attenuationDown(); // Call function attenuationDown, on press of key 'S'
	}

	if (key == 'Q' && action == GLFW_REPEAT)
	{
		scene->specularUp(); // Call function specularUp, on press of key 'Q'
	}
	if (key == 'W' && action == GLFW_REPEAT)
	{
		scene->specularDown(); // Call function specularDown, on press of key 'W'
	}
}
...
{ 
	...
	// Scene update '360' frames per second
	scene->update(1.0f / 360.f);
}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	glfwWindowHint(GLFW_SAMPLES, 8); // Antialiasing, sample eight times
